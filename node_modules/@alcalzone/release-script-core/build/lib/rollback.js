...
async function execGit(context, args) {
...
async function findStashIndexByMessage(context, stashMessage) {
    try {
...
async function dropRollbackStash(context, state) {
    if (!state.stashMessage)
        return;
 ...
 */
export async function captureRollbackSnapshot(context) {
    if (context.argv.dryRun)
        return;
    if (context.argv.noRollback)
        return;
    if (context.rollback)
    ...
    try {
        ...
        context.cli.warn(`Could not determine working tree status for rollback: ${e?.message ?? e}. ` +
    ...
    if (isDirty) {
        ...
        try {
            ...
            context.cli.warn(`Failed to snapshot uncommitted changes for rollback: ${e?.message ?? e}. ` +
                `Local rollback has been disabled for this run to avoid data loss if the release fails.`);
            return;
        }
        if (stashMessage) {
            ...
            try {
                ...
                throw new Error(`Could not re-apply your uncommitted changes after snapshotting them ` +
 ...
 */
export async function finalizeRollback(context, options) {
    if (context.argv.dryRun)
        return;
    if (context.argv.noRollback)
        return;
    ...
    if (state.pushAttempted) {
        context.cli.warn(`Skipping rollback because a push to the remote has already been attempted. ` +
    ...
    if (state.createdTag) {
        try {
            ...
            context.cli.warn(`Failed to delete release tag ${state.createdTag}: ${e?.message ?? e}`);
        }
    }
    ...
    if (state.stashMessage || state.stashSha) {
        ...
        if (ref) {
            try {
                ...
                context.cli.warn(`Failed to restore pre-release uncommitted changes: ${e?.message ?? e}. ` +
                    recoveryHint);
            }
        }
    }
