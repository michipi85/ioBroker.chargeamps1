...
export async function readDirAsZip(objects, id, name, options, callback) {
...
export async function writeDirAsZip(objects, id, name, data, options) {
    const zip = new JSZip();
    options = options || {};
    let adapter = id;
    if (adapter.includes('.')) {
...
// pack all files as zip and send it back
export async function readObjectsAsZip(objects, rootId, adapter, options = {}) {
    if (adapter) {
        // try to load processor of adapter
        try {
            options.stringify = (await import(`${tools.appName.toLowerCase()}.${adapter}/lib/convert.js`)).stringify;
...
export async function writeObjectsAsZip(objects, rootId, adapter, data, options, callback) {
    options = options || {};
    if (adapter) {
        // try to load processor of adapter
        try {
