...
class StatesInMemoryServer extends import_statesInMemFileDB.StatesInMemoryFileDB {
  ...
  constructor(settings) {
    ...
    this.open().then(() => {
      ...
      this.log.debug(`${this.namespace} ${settings.secure ? "Secure " : ""} Redis inMem-states listening on port ${this.settings.connection.port || 9e3}`);
      if (typeof this.settings.connected === "function") {
        setImmediate(() => this.settings.connected());
      }
    }).catch((e) => {
      this.log.error(`${this.namespace} Cannot start inMem-states on port ${this.settings.connection.port || 9e3}: ${e.message}`);
  ...
  _initRedisServer(settings) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        this.server.on("error", (err) => this.log.info(`${this.namespace} ${settings.secure ? "Secure " : ""} Error inMem-states listening on port ${settings.port || 9e3}: ${err}`));
