...
export class StateRedisClient {
    ...
    connectDb() {
        ...
        this.client.on('error', error => {
            this.settings.connection.enhancedLogging &&
                this.log.silly(`${this.namespace} Redis ERROR States: (${this.stop}) ${error.message} / ${error.stack}`);
            if (this.stop) {
                return;
            }
            if (!ready) {
        ...
        this.client.on('reconnecting', () => {
            ...
            this.settings.connection.enhancedLogging &&
                this.log.silly(`${this.namespace} States-Redis Event reconnect (reconnectCounter=${reconnectCounter}, stop=${this.stop})`);
        ...
        this.client.on('ready', async () => {
            ...
            if (isSentinel && wasReady) {
                this.log.info(`✅ ${this.namespace} States DB reconnected via Redis Sentinel (master group "${this.settings.connection.options.name}")`);
            }
            wasReady = true;
            let initCounter = 0;
            if (!this.subSystem && typeof onChange === 'function') {
                    ...
                    this.subSystem.on('pmessage', (pattern, channel, message) => {
                        setImmediate(() => {
                            ...
                            if (channel.startsWith(this.metaNamespace)) {
                                if (channel === `${this.metaNamespace}states.protocolVersion` &&
                ...
                this.subSystem.on('error', error => {
                    ...
                    this.settings.connection.enhancedLogging &&
                        this.log.silly(`${this.namespace} Sub-Client States System No redis connection: ${JSON.stringify(error)}`);
                });
                if (this.settings.connection.enhancedLogging) {
                    ...
                    this.subSystem.on('reconnecting', reconnectCounter => this.log.silly(`${this.namespace} PubSub client States-Redis System Event reconnect (reconnectCounter=${reconnectCounter}, stop=${this.stop})`));
                }
                this.subSystem.on('ready', async (_error) => {
                    ...
                    if (--initCounter < 1) {
                        if (this.settings.connection.port === 0) {
                            this.log.debug(`${this.namespace} States ${ready ? 'system re' : ''}connected to redis: ${tools.maybeArrayToString(this.settings.connection.host)}`);
                        }
                        else {
                            this.log.debug(`${this.namespace} States ${ready ? 'system re' : ''}connected to redis: ${tools.maybeArrayToString(this.settings.connection.host)}:${tools.maybeArrayToString(this.settings.connection.port)}`);
                        }
            ...
            if (!this.sub && typeof onChangeUser === 'function') {
                ...
                this.sub.on('ready', async (_error) => {
                    ...
                    if (--initCounter < 1) {
                        if (this.settings.connection.port === 0) {
                            this.log.debug(`${this.namespace} States ${ready ? 'user re' : ''}connected to redis: ${tools.maybeArrayToString(this.settings.connection.host)}`);
                        }
                        else {
                            this.log.debug(`${this.namespace} States ${ready ? 'user re' : ''}connected to redis: ${tools.maybeArrayToString(this.settings.connection.host)}:${tools.maybeArrayToString(this.settings.connection.port)}`);
                        }
                        !ready && typeof this.settings.connected === 'function' && this.settings.connected();
                        ready = true;
                    }
            ...
            if (initCounter < 1) {
                if (this.settings.connection.port === 0) {
                    this.log.debug(`${this.namespace} States ${ready ? 'client re' : ''}connected to redis: ${tools.maybeArrayToString(this.settings.connection.host)}`);
                }
                else {
                    this.log.debug(`${this.namespace} States ${ready ? 'client re' : ''}connected to redis: ${tools.maybeArrayToString(this.settings.connection.host)}:${tools.maybeArrayToString(this.settings.connection.port)}`);
                }
                !ready && typeof this.settings.connected === 'function' && this.settings.connected();
                ready = true;
            }
    ...
    async subscribe(pattern, asUser, callback) {
        ...
        try {
            ...
            if (asUser) {
                this.userSubscriptions[this.namespaceRedis + pattern] = new RegExp(tools.pattern2RegEx(this.namespaceRedis + pattern));
            }
            else {
                this.systemSubscriptions[this.namespaceRedis + pattern] = new RegExp(tools.pattern2RegEx(this.namespaceRedis + pattern));
            }
            return tools.maybeCallback(callback);
        }
        catch (e) {
